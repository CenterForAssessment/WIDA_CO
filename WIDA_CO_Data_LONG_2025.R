...
###############################################################################
###                                                                         ###
###        Data prep and cleaning for Colorado WIDA/ACCESS  -  2025         ###
###                                                                         ###
###############################################################################
...
#' prior to running SGP analyses. For the 2025 ACCESS data preparation and
...
###   Load data
WIDA_CO_Data_LONG_2025 <-
    as.data.table(openxlsx::read.xlsx(
        "Data/Base_Files/WIDA_ACCESS_GRO_READIN_2025_v2.xlsx"
    ))
###   Tidy up data
# WIDA_CO_Data_LONG_2025[, SCALE_SCORE := as.numeric(SCALE_SCORE)]

##    Student Names
WIDA_CO_Data_LONG_2025[, FIRST_NAME := as.factor(FIRST_NAME)]
setattr(
    WIDA_CO_Data_LONG_2025$FIRST_NAME, "levels",
    sapply(levels(WIDA_CO_Data_LONG_2025$FIRST_NAME), SGP::capwords)
)

WIDA_CO_Data_LONG_2025[, LAST_NAME := as.factor(LAST_NAME)]
setattr(
    WIDA_CO_Data_LONG_2025$LAST_NAME, "levels",
    sapply(levels(WIDA_CO_Data_LONG_2025$LAST_NAME), SGP::capwords)
)
...
setattr(
    WIDA_CO_Data_LONG_2025$GENDER, "levels",
    sapply(levels(WIDA_CO_Data_LONG_2025$GENDER), SGP::capwords)
)
...
setattr(
    WIDA_CO_Data_LONG_2025$ETHNICITY, "levels",
    sapply(levels(WIDA_CO_Data_LONG_2025$ETHNICITY), SGP::capwords)
)
...
WIDA_CO_Data_LONG_2025[, SCHOOL_NAME := as.factor(SCHOOL_NAME)]
setattr(
    WIDA_CO_Data_LONG_2025$SCHOOL_NAME, "levels",
    sapply(levels(WIDA_CO_Data_LONG_2025$SCHOOL_NAME), SGP::capwords)
)
...
###   Set key and invalidate duplicate cases  --  0 cases in 2025
# table(WIDA_CO_Data_LONG_2025[, VALID_CASE])
# setkey(WIDA_CO_Data_LONG_2025,
#        VALID_CASE, CONTENT_AREA, YEAR, GRADE, ID, SCALE_SCORE)
# setkey(WIDA_CO_Data_LONG_2025,
#        VALID_CASE, CONTENT_AREA, YEAR, GRADE, ID)
# WIDA_CO_Data_LONG_2025[
#     which(duplicated(WIDA_CO_Data_LONG_2025, by = key(WIDA_CO_Data_LONG_2025)))-1,
...
###   Save results
save(WIDA_CO_Data_LONG_2025, file = "Data/WIDA_CO_Data_LONG_2025.Rdata")
